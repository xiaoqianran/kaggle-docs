#!/usr/bin/env node
/**
 * Build zh-CN machine translations for Kaggle docs (English source of truth).
 *
  ...
  for (let attempt = 0; attempt < 4; attempt++) {
    try {
      const res = await fetch(url, {
        headers: {
          "User-Agent":
            "Mozilla/5.0 (compatible; kaggle-docs-mirror/1.0; +https://github.com/xiaoqianran/kaggle-docs)",
        },
      });
  ...
  await mapPool(limited, CONCURRENCY, async (abs) => {
    ...
    try {
      ...
      if (!fs.existsSync(zhAbs)) {
        ...
        fs.writeFileSync(
          zhAbs,
          "<!-- kaggle-docs: translation failed; English fallback -->\n\n" + en,
        );
        manifest.files[rel] = {
          hash,
          translatedAt: new Date().toISOString(),
