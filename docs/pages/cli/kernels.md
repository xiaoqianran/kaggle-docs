...
*   `<KERNEL>`: Kernel URL suffix (format: `owner/kernel-slug` or `owner/kernel-slug/version`, e.g., `kerneler/sqlite-global-default` or `kerneler/sqlite-global-default/2`).
...
*   `<KERNEL>`: Kernel URL suffix (format: `owner/kernel-slug` or `owner/kernel-slug/version`, e.g., `kerneler/using-google-bird-vocalization-model` or `kerneler/using-google-bird-vocalization-model/2`). Without a version, the latest version's output is downloaded.
...
Download the output of version 2 instead of the latest version:
